...
void BinaryOpExpr::generate_code(std::ostream& os, Context& context) const {
    ...
    int l_result_reg = 0;
    // os << "# BOP lhs got result reg t"<<l_result_reg<<std::endl;
    lhs->result_reg = l_result_reg;

    lhs->generate_code(os, context);

    int r_result_reg = 1;
    // os << "# BOP rhs got result reg t"<<r_result_reg<<std::endl;
    ...
    if (auto id_rhs = dynamic_cast<const IdExpr*>(rhs.get())) {
        ...
        // 直接加载常量到临时寄存器
        os << "  li t" << r_result_reg << ", " << int_rhs->get_value() << std::endl;
        avoid_stack = true;
    }
    ...
    if (!avoid_stack) {
        ...
        os << "  sw t"<<result_reg<<", 0(sp)" << std::endl;
        rhs->generate_code(os, context);
        os << "  add t" << r_result_reg << ", zero, t0" << std::endl;
        os << "  lw t" << result_reg <<", 0(sp)" << std::endl;
        os << "  addi sp, sp, 4" << std::endl;
    }
...
void AssignExpr::generate_code(std::ostream& os, Context& context) const {
    ...
    if(context.optimize_variable_cache){
        ...
        if(var_reg != -1){
            os << "# "<<var_id<<" value changed, update cache to t" << var_reg << std::endl;
            // context.mark_register_free(var_reg);
...
void DeclStmt::generate_code(std::ostream& os, Context& context) const {
    ...
    if(expr != nullptr){
        ...
        if(context.is_const.find(var_id) != context.is_const.end() && context.is_const[var_id]){
            ...
            // os << "# TODO: calculate const val " << var_id <<"="<<const_val<< std::endl;
            if (const_val == 0) {
                ...
                if(!unused){
                    os<<"# decl: cached "<<var_id<<" to reg t"<<result_reg<<std::endl;
                    context.mark_register_free(result_reg);
                    context.mark_register_used(result_reg, var_id);
                }
            }
...
void FunctionDecl::generate_code(std::ostream& os, Context& context) const {
    ...
    for (size_t i = 0; i < parameters.size(); ++i) {
        ...
        if (i < 8) {
            ...
            os << "  # Parameter " << param->get_name() << " saved from a" << i << std::endl;
        } else {
            os << "  lw t0, " << -((i - 8) * 4 + 4) + frame_size + (parameters.size() - 8) * 4<< "(sp)" << std::endl;
            os << "  sw t0, " << param_offset << "(s0)" << std::endl;
            os << "  # Parameter " << param->get_name() << " saved from stack" << i << std::endl;
        }
    ...
    os << "  addi sp, sp, " << frame_size << std::endl;
    if (return_type == "int") {
        // os << "  li a0, 0" << std::endl; // Default return 0 for int functions
...
void FunctionCallExpr::generate_code(std::ostream& os, Context& context) const {
    os << "  # Function call: " << function_name << std::endl;
    os << "  # Should clear regs used in this func: ";
    int temp = 0;
    ...
    os << "  # Save caller-saved registers" << std::endl;
    
    if(arguments.size()>8){
        os << "  addi sp, sp, -" << (arguments.size() - 8) * 4 << std::endl;
    }
    // Evaluate arguments and pass them (simplified - using stack for now)
    for (size_t i = 0; i < arguments.size(); ++i) {
        arguments[i]->generate_code(os, context);
        // For simplicity, we'll use a0, a1, a2, etc. for first few arguments
        if (i < 8) {
            os << "  mv a" << i << ", t0" << std::endl;
        } else {
            // For more than 8 arguments, we'd need to use the stack
    ...
    // Result is in a0, move to t0 for consistency
    ...
    os << "  # Restore caller-saved registers" << std::endl;
}
