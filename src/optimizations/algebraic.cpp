...
std::unique_ptr<Expr> AlgebraicPass::optimize_binary_op(BinaryOpExpr* expr, Context& context) {
    ...
    if (op == "+") {
        ...
        // 检查 x - x
        if (auto id_lhs = dynamic_cast<IdExpr*>(expr->get_lhs())) {
            if (auto id_rhs = dynamic_cast<IdExpr*>(expr->get_rhs())) {
...
bool AlgebraicPass::is_constant_foldable(Expr* expr) const {
    if (!expr) return false;
    
    if (auto binary_op = dynamic_cast<BinaryOpExpr*>(expr)) {
        return binary_op->get_lhs()->is_constant() && binary_op->get_rhs()->is_constant();
    } else if (auto unary_op = dynamic_cast<UnaryOpExpr*>(expr)) {
        return unary_op->get_expr()->is_constant();
    }
